...
type getCodeResult = ReturnType<typeof getCode>;
...
const getCodes = (time: string, arg: string | Record<string, string>) => {
  switch (typeof arg) {
    ...
    case 'object': {
      ...
      }, {} as Record<string, getCodeResult>);
...
export default (arg: string | Record<string, string>) => {
  const time = String(Math.round(Date.now() / 1000));
  const res = getCodes(time, arg);

  type Result = typeof arg extends string
    ? getCodeResult
    : Record<string, getCodeResult> & {
        time: string;
      };
